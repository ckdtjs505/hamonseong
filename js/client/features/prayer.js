...
async function checkAndRestoreSavedCompletion(dateStr) {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}api/hamonseong/get_completions.php?date=${dateStr}`, { credentials: 'include' });
    if (!res.ok) return;
...
async function deletePrayerRecord(id) {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}api/hamonseong/delete_completion.php`, {
      credentials: 'include',
      method: 'POST',

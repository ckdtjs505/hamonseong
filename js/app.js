...
async function loadBibleReading(dateStr) {
  ...
  try {
    // Step 1: 오늘의 읽기 계획 조회
    const planRes = await fetch(`${API_BASE_URL}api/get_plan.php?date=${dateStr}`, { credentials: 'include' });
    if (!planRes.ok) {
    ...
    const wordPromises = plans.map(plan => {
      ...
      return fetch(`${API_BASE_URL}api/get_word.php?book=${bookId}&start=${start}&end=${end}`, { credentials: 'include' })
        .then(res => {
